...
namespace AppAgenceVoyage2025.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.EmailUtilisateur, Email = model.EmailUtilisateur };

                var client = new Client();
                client.PreomUtilisateur = model.PreomUtilisateur;
                client.NomUtilisateur = model.NomUtilisateur;
                client.TelephoneUtilisateur = model.TelephoneUtilisateur;
                client.EmailUtilisateur = model.EmailUtilisateur;
                client.CNIClient = model.CNIClient;
                // DateTime.Now.ToString()
                var result = await UserManager.CreateAsync(user, "Passer123_lala");
                if (result.Succeeded)
                {
